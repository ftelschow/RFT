...
LKC_integral = function( R, coords = NULL, mask = NULL, h = 0.01, plot = F ){
  ...
  if( D == 1 ){
    ...
    if( is.null( mask ) ){
      mask <- list()
      mask[[1]] <- rbind( 1:length(x), seq( 0, 1, length.out = length(x) ) )
    }
    
    # Number of connected components
    nComp = length( mask )
    if( nComp == 0 ) return( function(u) return(0) )
      ...
      for( j in 1:nn ){
        if( j == 1 ){
          SC[[i]] = fields::interp.surface(
                      list( x = x, y = y, z = R[,,j] ),
                      cbind( mask$bdry[[i]]$x, mask$bdry[[i]]$y )
                    )
        }else{
    ...
    L[2] <- polyCub.midpoint( mask, f = volform, plot = TRUE )
  }
